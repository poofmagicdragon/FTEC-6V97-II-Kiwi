...
def execute_purchase_order():
    ...
    if portfolio is None:
        return jsonify(ErrorResponse(error_message = f'Portfolio with ID {portfolio_id} does not exist', request_id = g.request_id)), 500
    ...
    if current_caller not in authorized_users:
        return jsonify(ErrorResponse(error_message = f'User {current_caller} is not authorized to perform this activity on portfolio with ID {portfolio_id}', request_id = g.request_id)), 403
...
def liquidate_investment():
    ...
    if portfolio is None:
        return jsonify(ErrorResponse(error_message = f'Portfolio with ID {portfolio_id} does not exist', request_id = g.request_id)), 500
    managers = [security.username for security in portfolio.portfolio_securities if security.role == ['manager']]
    authorized_users = managers.append(portfolio.owner)
    current_caller = g.user('username')
    if current_caller not in authorized_users:
        return jsonify(ErrorResponse(error_message = f'User {current_caller} is not authorized to perform this activity on portfolio with ID {portfolio_id}', request_id = g.request_id)), 403
